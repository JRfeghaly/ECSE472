...
def parse_components(lines):
    ...
    for line in lines:
        ...
        if ctype == "V":
            ...
            comps.append({"type": "D", "name": name, "n1": n1, "n2": n2})
...
def nonlinear_dc_analysis(components, node_map, vsrc_names, N, M, max_iter=100, tol=1e-6):
    ...
    for iteration in range(max_iter):
        G, C, G2, Ectrl, b = stamp_components(components, node_map, vsrc_names, N, M)

        # Add diode contribution
        for c in components:
            if c["type"].upper() == "D":
                n1 = get_index(c["n1"], node_map)
                n2 = get_index(c["n2"], node_map)
                ...
                Vd = 0
                if n1 is not None: Vd += v[n1, 0]
                if n2 is not None: Vd -= v[n2, 0]
        ...
        if np.allclose(x_new, v, atol=tol):
...
def main():
    ...
    try:
        x = nonlinear_dc_analysis(comps, node_index, vsrc_names, N, M)
